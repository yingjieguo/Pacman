...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    for p in livingGhostPositionDistributions:
      highProbLocation = p.argMax()
      dist = self.distancer.getDistance(pacmanPosition, highProbLocation)
      if dist < mini:
        mini = dist
        closestGhost = highProbLocation

    actionNext = None
    mini = float("inf")
    for action in legal:
      dist = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), highProbLocation)
      if dist < mini:
        mini = dist
        actionNext = action
